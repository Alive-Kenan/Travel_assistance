...
describe("enrichSpotWithKimi", () => {
  ...
  test("preserves assistant tool call payload shape during builtin web search round-trip", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce({
        ok: true,
    ...
    const secondRequest = fetchMock.mock.calls[1]?.[1] as { body: string }
    expect(secondRequest.body).toContain("\"type\":\"builtin_function\"")
    expect(secondRequest.body).toContain("\"tool_call_id\":\"call_1\"")
    expect(secondRequest.body).toContain("\"content\":\"{\\\"query\\\":\\\"雁荡山 门票 交通\\\",\\\"total_tokens\\\":1234}\"")
  })

  test("includes raw user text and suggested search queries in the first enrichment request", async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: vi.fn().mockResolvedValue({
        choices: [
    ...
    expect(firstRequest.body).toContain("以下是用户提供的原始文本")
    expect(firstRequest.body).toContain("雁荡山 门票 住宿 美食")
    expect(firstRequest.body).toContain("建议优先围绕这些查询联网搜索")
    expect(firstRequest.body).toContain("雁荡山 门票")
    expect(firstRequest.body).toContain("雁荡山 住宿")
    expect(firstRequest.body).toContain("雁荡山 美食")
  })

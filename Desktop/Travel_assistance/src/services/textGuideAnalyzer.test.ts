...
describe("analyzeTextGuide", () => {
  ...
  it("supports short keyword input", async () => {
    ...
    expect(enrichSpotWithKimiMock).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
        rawUserText: "雁荡山 门票 住宿",
      }),
    )
    expect(enrichSpotWithKimiMock).toHaveBeenNthCalledWith(
      ...
      expect.anything(),
      expect.objectContaining({
        prioritySections: ["ticketPolicy", "stayGuide"],
      }),
    )
  })
  ...
  it("prioritizes user-requested sections during focused retries", async () => {
    enrichSpotWithKimiMock
      .mockResolvedValueOnce({
        source: { rawInput: "demo", kind: "text" },
        coreSpot: {
    ...
    await analyzeTextGuide("雁荡山 门票 住宿 美食")
  ...
  it("still retries when the first text result only has a title but no usable sections", async () => {
    enrichSpotWithKimiMock
      .mockResolvedValueOnce({
        source: { rawInput: "demo", kind: "text" },
        coreSpot: {
    ...
    expect(enrichSpotWithKimiMock).toHaveBeenCalledTimes(2)
    expect(result.guide.coreSpot.summary).toContain("适合一日精华游")
    expect(result.guide.highlights[0]?.title).toBe("灵峰夜景")
    expect(result.partialMessage).toBeNull()
  })

...
const VIDEO_FALLBACK_SUMMARY = "当前仅提取到部分视频信息，可结合联网补充进一步确认。"
const TEXT_FALLBACK_SUMMARY = "当前仅提取到部分文本线索，可结合联网补充进一步确认。"

export default function SpotGuideCards({ guide, mode = "full" }: Props) {
  const [copied, setCopied] = useState(false)
  const isPartial = mode === "partial"
  const isInferred = mode === "inferred"
  const isTextGuide = guide.source.kind === "text"
  const hasExtraInfo =
  ...
  const fallbackTagText = isTextGuide ? "文本信息仍在补充中" : "视频提取信息有限"
  const emptyStateCopy = {
    highlights: isTextGuide ? "暂未整理出景点亮点。" : "暂未从视频中提取到景点亮点。",
    checkpoints: isTextGuide ? "暂未整理出必打卡点。" : "暂未从视频中提取到必打卡点。",
    foodAndSouvenirs: isTextGuide ? "暂未整理出小吃或文创信息。" : "暂未从视频中提取到小吃或文创信息。",
    nearbyRecommendations: isTextGuide ? "暂未整理出周边推荐。" : "暂未从视频中提取到周边推荐。",
  }
  const summaryText =
    isTextGuide && guide.coreSpot.summary === VIDEO_FALLBACK_SUMMARY
      ? TEXT_FALLBACK_SUMMARY
      : guide.coreSpot.summary
      ...
      {isPartial ? (
        <SectionCard className="border-brand-orange/20 bg-brand-orange/10 shadow-none">
          <div className="text-sm text-ink-700">
            {isTextGuide
              ? "当前展示的是文本解析版结果，部分栏目仍在联网补充中；补全成功后会展示更完整的信息。"
      ...
      <SectionCard>
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
          <div className="min-w-0 max-w-full">
            ...
            <div className="mt-2 break-words text-sm leading-relaxed text-ink-500">{summaryText}</div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {guide.coreSpot.tripTags.length > 0 ? (
            ...
            <Tag tone="neutral">{fallbackTagText}</Tag>
          )}
        </div>
      </SectionCard>
      ...
      <SectionCard>
        ...
        <div className="mt-4 space-y-2">
          {guide.highlights.length > 0 ? (
            ...
            <EmptyState text={emptyStateCopy.highlights} />
          )}
        </div>
      </SectionCard>
      ...
      <SectionCard>
        ...
        <div className="mt-4 space-y-2">
          {guide.checkpoints.length > 0 ? (
            ...
            <EmptyState text={emptyStateCopy.checkpoints} />
          )}
        </div>
      </SectionCard>
        ...
        <SectionCard className="h-full">
          ...
          <div className="mt-4 space-y-2">
            {guide.foodAndSouvenirs.length > 0 ? (
              ...
              <EmptyState text={emptyStateCopy.foodAndSouvenirs} />
            )}
          </div>
        </SectionCard>
        ...
        <SectionCard className="h-full">
          ...
          <div className="mt-4 space-y-2">
            {guide.nearbyRecommendations.length > 0 ? (
              ...
              <EmptyState text={emptyStateCopy.nearbyRecommendations} />
            )}
          </div>
        </SectionCard>
      </div>
